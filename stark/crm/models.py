...
class ClassList(models.Model):
    ...
    tutor = models.ForeignKey(verbose_name='班主任', to='UserInfo', related_name='classes',
                              limit_choices_to={'depart__title': '教质部'}, on_delete=True)
...
class Customer(models.Model):
    ...
    consultant = models.ForeignKey(verbose_name="课程顾问", to='UserInfo', related_name='consultant', null=True, blank=True,on_delete=True)
    education_choices = (
        (1, '重点大学'),
        (2, '普通本科'),
        (3, '独立院校'),
...
class PaymentRecord(models.Model):
    ...
    缴费记录
    """
    customer = models.ForeignKey(Customer, verbose_name="客户",on_delete=True)
    consultant = models.ForeignKey(verbose_name="课程顾问", to='UserInfo', help_text="谁签的单就选谁",on_delete=True)
    class_list = models.ForeignKey(verbose_name="分配班级", to="ClassList", null=True, blank=True,on_delete=True)
    pay_type_choices = [
        (1, "报名费"),
        (2, "学费"),
        (3, "转班"),
    ...
    confirm_user = models.ForeignKey(verbose_name="确认人", to='UserInfo', related_name='confirms', null=True, blank=True,on_delete=True)
...
class ChangeClass(models.Model):
    ...
    转班记录
    """
    origin_class = models.ForeignKey(verbose_name="原班级", to="ClassList", related_name='x1',on_delete=True)
    target_class = models.ForeignKey(verbose_name="目标班级", to="ClassList", related_name='x2',on_delete=True)
    memo = models.TextField(verbose_name='原因')
    user = models.ForeignKey(verbose_name='处理人', to='UserInfo',on_delete=True)
...
class CourseRecord(models.Model):
    ...
    teacher = models.ForeignKey(verbose_name="讲师", to='UserInfo',on_delete=True)
